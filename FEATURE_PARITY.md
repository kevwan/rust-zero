...
| Service discovery | Broad coverage | Backend-neutral snapshots, dynamically balanced gRPC channels, endpoint metadata/weights, active probes, readiness, in-memory discovery, renewable etcd leases, and Kubernetes EndpointSlice recovery are covered. Etcd TLS/mTLS and automatic registration driven by `RpcServerConfig` remain open |
...
| REST | Broad coverage | Actix routing/extractors, HTTP clients, SSE, CORS primitives, auth, observability, recovery, request controls, validated assembly, default per-route result-aware circuit breaking, route policy, static/serverless serving, graceful draining, and content encryption are covered. TLS/mTLS and standard-stack CORS configuration remain open |
| gRPC | Broad coverage | Validated Tonic transport configuration, health, auth, discovery/balancing, tracing, deadlines, keepalives, client circuit breaking, default per-method result-aware server circuit breaking, adaptive shedding, metrics, streaming, and graceful drain are covered. TLS/mTLS, per-method configured server timeouts, and automatic etcd registration from `RpcServerConfig` remain open |
...
| MCP | Broad coverage | The 2025-03-26 Streamable HTTP transport provides stateless or expiring stateful sessions, JSON/SSE responses, resumable GET streams, termination, cancellation, dispatch, protocol errors, protection, startup, and graceful draining. The legacy 2024-11-05 SSE transport remains open for older clients |
