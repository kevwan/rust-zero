...
| REST | Broad coverage | Actix routing/extractors, HTTP clients, SSE, CORS primitives, TLS/mTLS, auth, observability, recovery, request controls, validated assembly, default per-route result-aware circuit breaking, route policy, static/serverless serving, graceful draining, and content encryption are covered. Standard-stack CORS configuration remains open |
| gRPC | Broad coverage | Validated Tonic TLS/mTLS transport configuration, health, auth, discovery/balancing, tracing, deadlines, keepalives, client circuit breaking, default per-method result-aware server circuit breaking, adaptive shedding, metrics, streaming, and graceful drain are covered. Per-method configured server timeouts and automatic etcd registration from `RpcServerConfig` remain open |
